function updateStudentCount() {
  fetch("http://45.13.119.138:8000/total_etudiant/1")
      ...
      .then(data => {
          ...
          document.getElementById("studentCount1").innerHTML = data.nombre_etudiant;
      })
      .catch(error => {
          console.error("Erreur lors de la requête API :", error);
      });
      // fetch("http://45.13.119.138:8000/total_etudiant/2")
      // .then(response => response.json())
      // .then(data => {
      //     // Met à jour le contenu de la balise avec l'ID "studentCount"
      //     document.getElementById("studentCount2").innerHTML = data.nombre_etudiant;
      // })
      // .catch(error => {
      //     console.error("Erreur lors de la requête API :", error);
      // });
